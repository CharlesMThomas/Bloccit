...
RSpec.describe Api::V1::UsersController, type: :controller do
  ...
  context "unathenticated users" do
    ...
    it "POST update returns http unauthenticated" do
      new_user = build(:user)
    ...
    it "POST update returns http unauthenticated" do
      new_user = build(:user)
      put :create, id: my_user.id, user: { name: new_user.name, email: new_user.email, password: new_user.password }
      expect(response).to have_http_status(401)
    end
  end

  context "authenticated and unathorized users" do
    before do
